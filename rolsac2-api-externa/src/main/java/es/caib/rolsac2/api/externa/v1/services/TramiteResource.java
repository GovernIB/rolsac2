...
public class TramiteResource {
	...
	public Response listarTramites(
			...
			@RequestBody(description = "Filtro de trámites: "
					+ FiltroTramite.SAMPLE, name = "filtro", content = @Content(example = FiltroTramite.SAMPLE_JSON, mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = FiltroTramite.class))) FiltroTramite filtro)
	...
	private RespuestaTramite getRespuesta(ProcedimientoTramiteFiltro filtro) throws DelegateException {
		...
		return new RespuestaTramite(Response.Status.OK.getStatusCode() + "", Constantes.mensaje200(lista.size()),
	...
	public Response listarDocumentos(
		...
		if (codigo != null) {
			...
			for (ProcedimientoDocumentoDTO nodo : result) {
				elemento = new ProcedimientoDocumento(nodo, null, idioma, true);
				lista.add(elemento);
			}
		}
