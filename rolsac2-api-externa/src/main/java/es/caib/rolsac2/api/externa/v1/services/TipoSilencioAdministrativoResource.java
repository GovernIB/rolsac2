...
@Path("/v1/" + Constantes.ENTIDAD_TIPO_SILENCIO)
@Tag(description = "/v1/" + Constantes.ENTIDAD_TIPO_SILENCIO, name = Constantes.ENTIDAD_TIPO_SILENCIO)
public class TipoSilencioAdministrativoResource {
	...
	@Path("/")
	@Operation(operationId = "llistarTiposSilenciosAdministrativos", summary = "Lista de tipos de silencios administrativos", description = "Lista todos los tipos de silencios administrativos disponibles")
	...
	public Response llistarTiposSilenciosAdministrativos(
	...
	private RespuestaTipoSilencioAdministrativo getRespuesta(TipoSilencioAdministrativoFiltro fg) throws DelegateException {
		...
		for (TipoSilencioAdministrativoDTO nodo : resultadoBusqueda.getItems()) {
			elemento = new TipoSilencioAdministrativo(nodo, null, fg.getIdioma(), true);
			lista.add(elemento);
		}
