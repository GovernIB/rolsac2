...
public abstract class EntidadBase<V> extends EntidadJson<V> {
    ...
    public List<Link> generaLink(List<Long> codigos, String entidad, String url, String urlBase, String descripcion) {
        ...
        if (codigos != null) {
            for (long codigo : codigos) {
                result.add(this.generaLink(codigo, entidad, url, urlBase, null));
            }
        }
    ...
    public Link generaLink(Long codigo, String entidad, String url, String urlBase, String descripcion) {
        if (codigo != null) {
            return new Link(entidad, codigo.toString(), (StringUtils.isEmpty(urlBase) ? Constantes.URL_BASE : urlBase + Constantes.URL_MODULO + Constantes.API_VERSION + "/") + url.replace("{0}", codigo + ""), descripcion, hateoasEnabled);
        }
        return null;
    }
