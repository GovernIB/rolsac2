...
/**
 * Dialogo de tasa de procedimiento/tramite.
 */
@Named
@ViewScoped
public class dialogTasasProcedimiento extends AbstractController implements Serializable {

    private static final Logger LOG = LoggerFactory.getLogger(dialogTasasProcedimiento.class);

    private TasaProcedimientoDTO data;

    public void load() {
        this.setearIdioma();

        if (this.isModoAlta()) {
            data = TasaProcedimientoDTO.createInstance(sessionBean.getIdiomasPermitidosList());
        } else if (this.isModoEdicion() || this.isModoConsulta()) {
            TasaProcedimientoDTO tasaMod = (TasaProcedimientoDTO) UtilJSF.getValorMochilaByKey("tasa");
            data = tasaMod.clone();
    ...
    private boolean verificarGuardar() {
        ...
        if (!data.getIdentificador().estaCompleto(sessionBean.getIdiomasObligatoriosList())) {
            UtilJSF.addMessageContext(TypeNivelGravedad.ERROR, getLiteral("dict.obligatorio.generico.literal", new Object[]{getLiteral("dialogTasasProcedimiento.codigo")}), true);
            return false;
        }
        if (data.getFormaPago() == null || data.getFormaPago().estaVacio()) {
            UtilJSF.addMessageContext(TypeNivelGravedad.ERROR, getLiteral("dict.obligatorio.formaPago"), true);
            return false;
        }
    ...
    public TasaProcedimientoDTO getData() {
        return data;
    }

    public void setData(TasaProcedimientoDTO data) {
        this.data = data;
    }
    ...
    public void returnDialogTraducir(final SelectEvent event) {
        if (event != null && event.getObject() instanceof TasaProcedimientoDTO) {
            this.data = ((TasaProcedimientoDTO) event.getObject());
        }
    }
}
