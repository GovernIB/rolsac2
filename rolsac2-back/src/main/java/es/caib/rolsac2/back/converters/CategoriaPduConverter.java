...
@Named
@ViewScoped
public class CategoriaPduConverter implements Converter, Serializable {
    ...
    public Object getAsObject(FacesContext facesContext, UIComponent uiComponent, String s) {
        ...
        if (s != null && (s.contains("Seleccioni un") || s.contains("Seleccione un") || s.equals("Seleccione una categoría") || s.equals("Seleccioni un categoria"))) {
            s = null;
        }

        if (s != null && s.trim().length() > 0) {
            return pduServiceFacade.findCategoriaById(Long.parseLong(s));
        } else {
            return null;
        }
    }
