...
public class ProcedimientoResource {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(ProcedimientoResource.class);
    ...
    public Response listarProcedimientos(
        ...
        if (ordenCampo != null && !ordenCampo.trim().isEmpty()) {
            String orderByInterno = mapOrdenCampo(ordenCampo.trim());
            if (orderByInterno != null) {
                fg.setOrderBy(orderByInterno);
            } else {
                LOG.warn("El parámetro ordenCampo no es válido: {}", ordenCampo);
            }
        }
    ...
    /**
     * Traduce los nombres públicos definidos por el contrato REST a los nombres
     * de propiedad usados por el modelo/filtro interno.
     */
    private String mapOrdenCampo(final String ordenCampo) {
        if (ordenCampo == null) {
            return null;
        }
        switch (ordenCampo) {
            ...
            case "dataActualitazio": // errata que aparece en el documento de análisis
    ...
    private RespuestaBase getRespuesta(final ProcedimientoFiltro filtro, final String idiomaPorDefecto, final Instant start, String url, Integer apiMaxLimit) {
        ...
        // IMPORTANTE: el contrato exige devolver sólo versiones definitivas (publicadas o cerradas).
        // Esta restricción debe aplicarse dentro de findProcedimientosByFiltroRest para que total/paginación sean coherentes.
