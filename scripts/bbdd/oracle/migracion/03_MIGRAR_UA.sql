...
BEGIN
	...
	THEN
		...
		/** SI EL PADRE NO ES NULO, COMPROBEMOS. **/
		IF CODIGO_PADRE IS NOT NULL
		THEN
			SELECT COUNT(*)
			INTO EXISTE_PADRE
			FROM RS2_UNIADM
			WHERE UNAD_CODIGO = CODIGO_PADRE;

			IF EXISTE_PADRE = 0
			THEN
				/** NO SE SIGUE PQ DARÁ ERROR, SE OBTIENE EL MENSAJE DE LA FUNCION Y SE PARA **/
				resultado := CHECK_ERROR_UA(CODIGO_PADRE, codigoUA, NOMBRE);
				rollback;
				RETURN;
			END IF;
		END IF;
