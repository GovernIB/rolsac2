...
AS
	...
	***/
	...
	usu_codigo_r1  NUMBER;
	usu_codigo_r2  NUMBER;
	codigoUA       NUMBER;
...
BEGIN
	...
	/** Extraemos el codigo en R1 y R2 **/
	SELECT usu_codi
	INTO usu_codigo_r1
	FROM R1_USUARIO
	WHERE USU_USERNA = usuario  AND ROWNUM = 1;

	SELECT user_codigo
	INTO usu_codigo_r2
	FROM RS2_USER
	WHERE USER_USER = usuario  AND ROWNUM = 1;
	...
	FOR ROLSAC1_USERUA IN cursorUserUAsROLSAC1(usu_codigo_r1)
		LOOP
			...
			/** si no se ha introducido la relacion UA - User y existe la UA en ROLSAC2, entonces introducir **/
			IF  EXISTE_UA_USER = 0 AND EXISTE_UA_ROLSAC2 = 1
			THEN
				INSERT INTO RS2_USERUA (UAUS_CODUA,UAUS_CODUSER )
				VALUES (codigoUA,usu_codigo_r2 );
			END IF;
