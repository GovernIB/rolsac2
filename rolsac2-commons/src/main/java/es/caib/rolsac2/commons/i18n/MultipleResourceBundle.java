...
public class MultipleResourceBundle extends ResourceBundle {
    ...
    protected Object handleGetObject(String key) {
        Optional<Object> firstPropertyValue = this.delegates.stream().filter(delegate -> delegate.containsKey(key)).map(delegate -> delegate.getObject(key)).findFirst();
    ...
    public Enumeration<String> getKeys() {
        List<String> keys = this.delegates.stream().flatMap(delegate -> Collections.list(delegate.getKeys()).stream()).distinct().collect(Collectors.toList());
