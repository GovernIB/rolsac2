...
public class ProcedimientoServiceFacadeBean implements ProcedimientoServiceFacade {
	...
	public Long create(ProcedimientoBaseDTO dto, TypePerfiles perfil)
			throws RecursoNoEncontradoException, DatoDuplicadoException {
		return create(dto, perfil, true);
	}

	private Long create(ProcedimientoBaseDTO dto, TypePerfiles perfil, boolean conAuditoria)
			throws RecursoNoEncontradoException, DatoDuplicadoException {
		...
		procedimientoRepository.mergeDocumentos(jProcWF.getCodigo(),
				jProcWF.getListaDocumentos() == null ? null : jProcWF.getListaDocumentos().getCodigo(), false,
				dto.getDocumentos(), ruta);
		procedimientoRepository.mergeDocumentos(jProcWF.getCodigo(),
				jProcWF.getListaDocumentosLOPD() == null ? null : jProcWF.getListaDocumentosLOPD().getCodigo(), true,
				dto.getDocumentosLOPD(), ruta);
		if (dto instanceof ProcedimientoDTO) {
		...
		ProcedimientoBaseDTO dtoAntiguo = ProcedimientoBaseDTO
				.createInstance(dto.getNombreProcedimientoWorkFlow().getIdiomas());
		if (conAuditoria) {
	...
	private List<JProcedimientoWorkflowTraduccion> crearTraducciones(ProcedimientoBaseDTO dto,
			JProcedimientoWorkflow jProcWF) {
	...
	private void mergear(JProcedimientoWorkflow jProcWF, ProcedimientoBaseDTO dto) {
		...
		jProcWF.setDatosPersonalesLegitimacion(
				tipoLegitimacionConverter.createEntity(dto.getDatosPersonalesLegitimacion()));
		...
		if (dto instanceof ServicioDTO) {
			...
			if (((ServicioDTO) dto).getPlantillaSel() != null || ((ServicioDTO) dto).getTipoTramitacion() != null) {
				...
				if (((ServicioDTO) dto).getPlantillaSel() != null
						&& ((ServicioDTO) dto).getPlantillaSel().getCodigo() != null) {
					jTipoTramitacion = tipoTramitacionRepository
							.findById(((ServicioDTO) dto).getPlantillaSel().getCodigo());
					jProcWF.setTramiteElectronicoPlantilla(jTipoTramitacion);
					if (jProcWF.getTramiteElectronico() != null
							&& jProcWF.getTramiteElectronico().getCodigo() != null) {
						tipoTramitacionRepository.borrarByServicio(jProcWF.getCodigo(),
								jProcWF.getTramiteElectronico().getCodigo());
					}
					...
					if (((ServicioDTO) dto).getTipoTramitacion().getCodigo() == null) {
						jTipoTramitacion = tipoTramitacionConverter
								.createEntity(((ServicioDTO) dto).getTipoTramitacion());
						tipoTramitacionRepository.create(jTipoTramitacion);
					} else {
						jTipoTramitacion = tipoTramitacionRepository
								.findById(((ServicioDTO) dto).getTipoTramitacion().getCodigo());
						...
						if (((ServicioDTO) dto).getTipoTramitacion().getCodPlatTramitacion() == null) {
							...
							JPlatTramitElectronica jplataforma = platTramitElectronicaConverter
									.createEntity(((ServicioDTO) dto).getTipoTramitacion().getCodPlatTramitacion());
	...
	@RolesAllowed({ TypePerfiles.ADMINISTRADOR_CONTENIDOS_VALOR, TypePerfiles.ADMINISTRADOR_ENTIDAD_VALOR,
			TypePerfiles.SUPER_ADMINISTRADOR_VALOR, TypePerfiles.GESTOR_VALOR, TypePerfiles.INFORMADOR_VALOR })
	public void update(ProcedimientoBaseDTO dto, ProcedimientoBaseDTO dtoAntiguo, TypePerfiles perfil, Long idEntidad)
		...
		TypeIndexacion tipo = (dto instanceof ProcedimientoDTO) ? TypeIndexacion.PROCEDIMIENTO
				: TypeIndexacion.SERVICIO;
	...
	private void updateWF(ProcedimientoBaseDTO dto, JProcedimientoWorkflow jProcWF)
			throws RecursoNoEncontradoException {
		mergear(jProcWF, dto);
		...
		if (documento.getDocumentos() != null && documento.getDocumentos().getTraducciones() != null) {
			for (DocumentoTraduccion trad : documento.getDocumentos().getTraducciones()) {
				if (trad.getFicheroDTO() != null && trad.getFicheroDTO().getCodigo() != null) {
					FicheroDTO ficheroDTO = ficheroExternoRepository.getContentById(trad.getFicheroDTO().getCodigo(),
							ruta);
					Long idFicheroNuevo = ficheroExternoRepository.createFicheroExterno(ficheroDTO.getContenido(),
							ficheroDTO.getFilename(), ficheroDTO.getTipo(), null, ruta);
	...
	public List<TipoPublicoObjetivoEntidadDTO> getTipoPubObjEntByProc(Long codigo, String enlaceWF) {
		...
		if (jprocWF != null) {
			...
			return procedimientoRepository.getTipoPubObjEntByWFRest(listJprocWF.get(0).getCodigo(),
					listJprocWF.get(1).getCodigo(), enlaceWF);
		}
	...
	public List<ProcedimientoDocumentoDTO> getDocumentosByProc(Long codigo, String enlaceWF) {
		...
		if (jprocWF != null && jprocWF.getListaDocumentos() != null) {
			...
			return procedimientoRepository.getDocumentosByListaDocumentos(listJprocWF.get(0).getListaDocumentos(),
					listJprocWF.get(1).getListaDocumentos(), enlaceWF);
		}
	...
	public List<ProcedimientoDocumentoDTO> getDocumentosLOPDByProc(Long codigo, String enlaceWF) {
		...
		if (jprocWF != null && jprocWF.getListaDocumentosLOPD() != null) {
			...
			return procedimientoRepository.getDocumentosByListaDocumentos(listJprocWF.get(0).getListaDocumentosLOPD(),
					listJprocWF.get(1).getListaDocumentosLOPD(), enlaceWF);
		}
	...
	private ProcedimientoBaseDTO getProcedimientoDTOByCodigo(Long codigo) {
		...
		if (jprocWF.getDatosPersonalesLegitimacion() != null) {
			proc.setDatosPersonalesLegitimacion(
					tipoLegitimacionConverter.createDTO(jprocWF.getDatosPersonalesLegitimacion()));
		}
		...
		if (jprocWF.getTraducciones() != null) {
			for (JProcedimientoWorkflowTraduccion trad : jprocWF.getTraducciones()) {
				...
				datosPersonalesDestinatario
						.add(new Traduccion(trad.getIdioma(), trad.getDatosPersonalesDestinatario()));
		...
		proc.setDocumentosLOPD(
				procedimientoRepository.getDocumentosByListaDocumentos(jprocWF.getListaDocumentosLOPD()));
	 ...
	 */
	private void crearAuditoria(final ProcedimientoBaseDTO procedimientoAntiguo,
			final ProcedimientoBaseDTO procedimientoNuevo, TypePerfiles perfil, String literalFlujo, String accion) {
	...
	public void guardarFlujo(ProcedimientoBaseDTO data, TypeProcedimientoEstado estadoDestino, String mensajes,
			TypePerfiles perfil, boolean pendienteMensajeSupervisor, boolean pendienteMensajesGestor, Long idEntidad) {

		// Paso 0. Marcamos para indexar en solr/elastic y SIA.
		TypeIndexacion tipo = (data instanceof ProcedimientoDTO) ? TypeIndexacion.PROCEDIMIENTO
				: TypeIndexacion.SERVICIO;

		/** 2 == borrado ; 1 == insertar **/
		int accionIdx = (estadoDestino == TypeProcedimientoEstado.BORRADO
				|| estadoDestino == TypeProcedimientoEstado.RESERVA) ? 2 : 1;
		...
		if (TypeProcedimientoEstado.distintoWorkflow(data.getEstado(), estadoDestino)) {
			...
			Long codigoWF = procedimientoRepository.getCodigoByWF(data.getCodigo(),
					true);
			...
			if (procDestino == null && data.getCodigo() != null) {
				if(estadoDestino.equals(TypeProcedimientoEstado.BORRADO) || estadoDestino.equals(TypeProcedimientoEstado.RESERVA)) {
					generarAuditoria("auditoria.flujo." + data.getEstado().toString() + estadoDestino.toString(),
							data.getCodigo(), data.getUsuarioAuditoria(), perfil, TypeAccionAuditoria.BAJA.toString());
				} else {
					generarAuditoria("auditoria.flujo." + data.getEstado().toString() + estadoDestino.toString(),
							data.getCodigo(), data.getUsuarioAuditoria(), perfil, TypeAccionAuditoria.MODIFICACION.toString());
				}

			} else {
				if(estadoDestino.equals(TypeProcedimientoEstado.BORRADO) || estadoDestino.equals(TypeProcedimientoEstado.RESERVA)) {
					crearAuditoria(procDestino, data, perfil,
							"auditoria.flujo." + data.getEstado().toString() + estadoDestino.toString(), TypeAccionAuditoria.BAJA.toString());
				} else {
					crearAuditoria(procDestino, data, perfil,
							"auditoria.flujo." + data.getEstado().toString() + estadoDestino.toString(), TypeAccionAuditoria.MODIFICACION.toString());
				}
			}
		...
		procedimientoRepository.actualizarMensajes(data.getCodigo(), mensajes, pendienteMensajeSupervisor,
				pendienteMensajesGestor);
	...
	@RolesAllowed({ TypePerfiles.ADMINISTRADOR_CONTENIDOS_VALOR, TypePerfiles.ADMINISTRADOR_ENTIDAD_VALOR,
			TypePerfiles.SUPER_ADMINISTRADOR_VALOR, TypePerfiles.GESTOR_VALOR, TypePerfiles.INFORMADOR_VALOR })
	public void actualizarMensajes(Long idProc, String mensajes, boolean pendienteMensajeSupervisor,
			boolean pendienteMensajesGestor) {
		procedimientoRepository.actualizarMensajes(idProc, mensajes, pendienteMensajeSupervisor,
				pendienteMensajesGestor);
	}
	...
	@RolesAllowed({ TypePerfiles.ADMINISTRADOR_CONTENIDOS_VALOR, TypePerfiles.ADMINISTRADOR_ENTIDAD_VALOR,
			TypePerfiles.SUPER_ADMINISTRADOR_VALOR, TypePerfiles.GESTOR_VALOR, TypePerfiles.INFORMADOR_VALOR })
	public DataIndexacion findDataIndexacionTram(ProcedimientoTramiteDTO tramite, ProcedimientoDTO procedimientoDTO,
			PathUA pathUA) {
	...
	@RolesAllowed({ TypePerfiles.ADMINISTRADOR_CONTENIDOS_VALOR, TypePerfiles.ADMINISTRADOR_ENTIDAD_VALOR,
			TypePerfiles.SUPER_ADMINISTRADOR_VALOR, TypePerfiles.GESTOR_VALOR, TypePerfiles.INFORMADOR_VALOR,
			TypePerfiles.RESTAPI_VALOR })
	public IndexFile findDataIndexacionProcDoc(ProcedimientoDTO procedimientoDTO, ProcedimientoDocumentoDTO doc,
			DocumentoTraduccion documentoTraduccion, PathUA pathUA) {
		String ruta = systemService.obtenerPropiedadConfiguracion(TypePropiedadConfiguracion.PATH_FICHEROS_EXTERNOS);
		FicheroDTO ficheroDTO = ficheroExternoRepository.getContentById(documentoTraduccion.getFicheroDTO().getCodigo(),
				ruta);
		return CastUtil.getDataIndexacion(procedimientoDTO, doc, documentoTraduccion, ficheroDTO,
				documentoTraduccion.getIdioma(), pathUA);
	}

	@Override
	@RolesAllowed({ TypePerfiles.RESTAPI_VALOR })
	public Pagina<ProcedimientoDocumentoDTO> findProcedimientoDocumentoByFiltroRest(
			ProcedimientoDocumentoFiltro filtro) {
		try {
	...
	@Override
	public IndexFile findDataIndexacionTramDoc(ProcedimientoTramiteDTO tramite, ProcedimientoDTO procedimientoDTO,
			ProcedimientoDocumentoDTO doc, DocumentoTraduccion fichero, PathUA pathUA) {
		String ruta = systemService.obtenerPropiedadConfiguracion(TypePropiedadConfiguracion.PATH_FICHEROS_EXTERNOS);
		FicheroDTO ficheroDTO = ficheroExternoRepository.getContentById(fichero.getFicheroDTO().getCodigo(), ruta);
		return CastUtil.getDataIndexacion(procedimientoDTO, tramite, doc, fichero, ficheroDTO, fichero.getIdioma(),
				pathUA);
	}
