...
import es.caib.rolsac2.api.interna.v1.model.TipoLegitimacion;
import es.caib.rolsac2.api.interna.v1.model.filters.FiltroTipoLegitimacion;
import es.caib.rolsac2.api.interna.v1.model.respuestas.RespuestaError;
import es.caib.rolsac2.api.interna.v1.model.respuestas.RespuestaTipoLegitimacion;
...
public class TipoLegitimacionResource {
     ...
     *
     * @return Listado de TiposLegitimacion
    ...
    /**
     * Para obtener el tipo legitimacion.
     *
     * @param codigo Código del tipo legitimacion
    ...
    private RespuestaTipoLegitimacion getRespuesta(TipoLegitimacionFiltro fg, Instant start) {
        Pagina<TipoLegitimacionDTO> resultadoBusqueda = tipoLegitimacionService.findByFiltroRest(fg);

        List<TipoLegitimacion> lista = new ArrayList<>();
        TipoLegitimacion elemento;

        for (TipoLegitimacionDTO nodo : resultadoBusqueda.getItems()) {
            elemento = new TipoLegitimacion(nodo, systemService.obtenerPropiedadConfiguracion(TypePropiedadConfiguracion.URL_BASE), fg.getIdioma(), true);
            lista.add(elemento);
        }
        ...
        return new RespuestaTipoLegitimacion(Response.Status.OK.getStatusCode() + "", Constantes.mensaje200(lista.size()), resultadoBusqueda.getTotal(), lista, tiempoMiliSegundos);
    }
}
