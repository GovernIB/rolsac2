...
import es.caib.rolsac2.api.interna.v1.model.respuesta.RespuestaServicios;
...
public class ServiciosResource {
    ...
    @APIResponse(responseCode = "200", description = Constantes.MSJ_200_GENERICO, content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = RespuestaServicios.class)))
    @APIResponse(responseCode = "400", description = Constantes.MSJ_200_GENERICO, content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = RespuestaError.class)))
    public Response listarServicios(@Parameter(description = "Código de idioma", name = "lang", in = ParameterIn.QUERY) @QueryParam("lang") final String lang,
                                    ...
                                    @Parameter(description = "Id entidad", name = "entidad", in = ParameterIn.QUERY) @QueryParam("entidad") final String idEntidad) throws ValidationException {

        Instant start = Instant.now();
        FiltroServicios filtro = new FiltroServicios();
        ...
        return Response.ok(getRespuesta(fg, idiomaPorDefecto, start, "listarServicios"), MediaType.APPLICATION_JSON).build();
    }
    ...
    @APIResponse(responseCode = "400", description = Constantes.MSJ_200_GENERICO, content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = RespuestaError.class)))
    public Response numeroServicios(@Parameter(description = "Código de idioma", name = "lang", in = ParameterIn.QUERY) @QueryParam("lang") final String lang,
                                    @RequestBody(description = "Filtro de servicios: " + FiltroServicios.SAMPLE, name = "filtro",
                                            content = @Content(example = FiltroServicios.SAMPLE_JSON, mediaType = MediaType.APPLICATION_JSON,
                                                    schema = @Schema(implementation = FiltroServicios.class))) FiltroServicios filtro) throws ValidationException {
    ...
    private RespuestaBase getRespuesta(final ProcedimientoFiltro filtro, String idiomaPorDefecto, Instant start, String nombreMetodo) {
        ...
        return RespuestaServicios.builder().pageSize(filtro.getPaginaTamanyo()).
                totalCount(resultadoBusqueda.getTotal()).itemsReturned("Servicios").page(filtro.getPaginaFirst()).
                dateDownload(formattedDate).name("Servicios." + nombreMetodo).data(lista).
    ...
    @APIResponse(responseCode = "400", description = Constantes.MSJ_400_GENERICO, content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = RespuestaError.class)))
    public Response listarNormativas(@Parameter(description = "Código servicio workflow", name = "codigo", required = true, in = ParameterIn.PATH) @PathParam("codigo") final String codigo,
